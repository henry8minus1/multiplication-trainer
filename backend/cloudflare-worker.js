...
export default{async fetch(request,env){
  ...
  if(request.method==='GET'&&url.pathname==='/health'){const provider=String(env.AI_PROVIDER||DEFAULT_PROVIDER).toLowerCase();return json({ok:true,provider,accessTokenConfigured:Boolean(env.MATH_APP_ACCESS_TOKEN),geminiKeyConfigured:Boolean(env.GEMINI_API_KEY),openAIKeyConfigured:Boolean(env.OPENAI_API_KEY),geminiModel:env.GEMINI_MODEL||DEFAULT_GEMINI_MODEL},200,origin)}
  if(request.method!=='POST'||!['/word-problem','/grade-session'].includes(url.pathname))return json({error:'not_found'},404,origin);
